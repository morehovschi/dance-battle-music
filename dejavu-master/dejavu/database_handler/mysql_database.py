...
class Cursor(object):
    ...
    def __init__(self, dictionary=False, **options):
        ...
        try:
            ...
            options.update({'charset': 'utf8mb4'})  # LINE ADDED
            conn = mysql.connector.connect(**options)
            conn.autocommit = True  # LINE ADDED
    ...
    def __exit__(self, extype, exvalue, traceback):
        ...
        if extype is DatabaseError:
            self.conn.rollback()  # Usamos rollback sobre la conexión, no sobre el cursor.

        self.cursor.close()
        # self.conn.commit() #LINE COMMENTED
