...
class ChunkProcessor:
    ...
    def query_and_reference_chunk_audio( self, row ):
        query_fpath = f"{self.audio_data_path}/{self.meta_df.iloc[ row ][ "Number" ]}"\
                      f"[{self.meta_df.iloc[row]['query\'s YouTubeID']}]/full.mp3"
        ...
        reference_fpath = f"{self.audio_data_path}/{self.meta_df.iloc[ row ][ 'Number' ]}"\
                          f"[{self.meta_df.iloc[ row ][ 'query\'s YouTubeID' ]}]/"\
                          f"reference[{reference_id}].mp3"
